...
getCoursesFor(userId).then(courses => {
  ...
  courses.forEach((currentCourse) => {
    ...
    console.log(percentageAssigned, subtractGrade, scoreNeeded)
    if (percentageAssigned == 0 && subtractGrade > 0) {
      newTextTargetNeeded.textContent = 'Target grade not achieved'
    }
    else if (percentageAssigned == 0 && subtractGrade <= 0) {
...
document.querySelector(`#upcomingAssignments`).addEventListener('click', () => {
  ...
  dueAsmtList.forEach((v) => {
    ...
    if (d == today) {
      const result = dueAsmtList.filter(obj => { return obj["Due Date"] == d })
      todayList.push(result);
    }
    else if (d == dueTomorrow) {
      const result = dueAsmtList.filter(obj => { return obj["Due Date"] == d })
      tomorrowList.push(result);
    }
    else if ((d > dueTomorrow) && (d < dueWeek)) {
      const result = dueAsmtList.filter(obj => { return obj["Due Date"] == d })
      thisWeekList.push(result);
    }
    else if (d >= dueWeek) {
      const result = dueAsmtList.filter(obj => { return obj["Due Date"] == d })
      laterList.push(result);
    }
    else {
  ...
  const getTableDetails = (list) => {
    for (const item of list) {
      ...
      item.forEach(function (asmt) {
        tdAssignment.textContent = asmt["Assignment"];
        tdCourseCode.textContent = asmt["Course Code"];
        tdDueDate.textContent = asmt["Due Date"];
      });
